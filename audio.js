...
function sfx(type,vol=.1){if(!ctx)return;const v=Math.min(.34,vol*2.5);
 if(type==='pistol'){noise(.045,v*1.3,950,'highpass');noise(.11,v*.78,1250,'bandpass',.003);tone(118,42,.16,v*.72,'sine');noise(.2,v*.32,420,'lowpass',.018);tone(1850,620,.022,v*.14,'triangle',.01)}
